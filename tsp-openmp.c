...
double lowerbound_estimate(int n_cities){
    ...
    double lowerbound=0;
    int i;
    #pragma omp parallel for reduction(+:lowerbound)
        for(i=0;i<n_cities;i++){
...
int tsp(int n_cities, double max_value){
    ...
    double *best_array=malloc(omp_get_num_threads()*sizeof(double));
    double best_value=1000000000;
    int k;
    int best_thread;

    double lowerbound = lowerbound_estimate(n_cities);
    int *bestTour;
...
#pragma omp parallel num_threads(4) firstprivate(bestTourCost,bestTourLength,newBound,newCost) private(i,bestTour,cf,ct,min1f,min2f,min1t,min2t)
{
    //printf("im alive [%d]\n",omp_get_thread_num());
    bestTour = (int*)malloc(n_cities * sizeof(int));
    ...
    while(queue->size != 0){
        ...
        if(element->bound >= bestTourCost || flag==1){
    ...
    best_array[omp_get_thread_num()]=bestTourCost;
    printf("[%d] waiting for others\n",omp_get_thread_num());
    #pragma omp barrier
    #pragma omp single
    {
        for(k=0;k<omp_get_num_threads();k++){
            //printf("this thread [%d] has this result [%.2lf]\n",k,best_array[k]);
            if(best_array[k]<best_value){
                best_value=best_array[k];
                best_thread=k;
            }
    ...
    if(omp_get_thread_num()==best_thread){
        //printf("i [%d] have the best result: %.2lf\n",best_thread,bestTourCost);
